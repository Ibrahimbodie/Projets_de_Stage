from __future__ import annotations

from typing import List

from langchain_core.documents import Document
from langchain_core.prompts import PromptTemplate

from llm import create_llm, llm_invoke
from config import load_config


FALLBACK_MESSAGE = (
    "Information non trouvée dans les règlements officiels."
)


def build_prompt() -> PromptTemplate:
    template = (
        "Tu es un assistant académique intelligent "
        "du Master SIM à VNU-IS.\n\n"
        "Tu aides les étudiants à comprendre "
        "les règlements académiques.\n\n"
        "RÈGLES IMPORTANTES :\n"
        "- Réponds directement.\n"
        "- Ne répète jamais la question.\n"
        "- Ne dis jamais : "
        "'la question est', "
        "'l'étudiant demande', "
        "'vous avez demandé'.\n"
        "- Utilise uniquement les informations "
        "présentes dans le contexte.\n"
        "- Reformule avec des phrases simples.\n"
        "- Explique clairement comme à un étudiant.\n"
        "- Donne une réponse courte, naturelle "
        "et professionnelle.\n"
        "- Ne fais pas d'introduction inutile.\n"
        "- Ne fais pas de conclusion inutile.\n"
        "- Si l'information n'existe pas dans "
        "le contexte, répond exactement :\n"
        "'Information non trouvée dans "
        "les règlements officiels.'\n\n"
        "CONTEXTE :\n"
        "{context}\n\n"
        "QUESTION :\n"
        "{question}\n\n"
        "RÉPONSE DIRECTE :"
    )
    return PromptTemplate.from_template(template)


def format_context(documents: List[Document]) -> str:
    parts = []
    for doc in documents:
        page = doc.metadata.get("page")
        page_label = (
            f"Page {page + 1}"
            if isinstance(page, int)
            else "Page inconnue"
        )
        clean_text = " ".join(doc.page_content.split())
        parts.append(f"[{page_label}]\n{clean_text}")
    return "\n\n".join(parts)


def format_sources(documents: List[Document]) -> List[str]:
    pages = []
    for doc in documents:
        page = doc.metadata.get("page")
        if isinstance(page, int):
            pages.append(page + 1)
    pages = sorted(set(pages))
    return [f"Page {page}" for page in pages]


def clean_generated_answer(answer: str) -> str:
    import re

    apos = r"[''ʼ']"
    unwanted_patterns = [
        rf"La question de l{apos}étudiant est\s*:",
        rf"La question posée est\s*:",
        rf"L{apos}étudiant demande\s*:",
        rf"Vous avez demandé\s*:",
        r"Question\s*:",
        r"[Rr][eéèêë]ponse [pP][eéèêë]dagogique\s*:",
        r"[Rr][eéèêë]ponse\s*:",
    ]
    cleaned = answer.strip()
    for pattern in unwanted_patterns:
        cleaned = re.sub(pattern, "", cleaned, flags=re.IGNORECASE)
    return cleaned.strip()


def generate_answer(
    question: str,
    documents: List[Document],
    config=None,
) -> str:
    if not documents:
        return FALLBACK_MESSAGE

    prompt = build_prompt()
    context = format_context(documents)

    if config is None:
        cfg = load_config()
        config = cfg.llm

    llm = create_llm(
        provider=config.provider,
        model=config.model,
        temperature=getattr(config, "temperature", 0.2),
        num_predict=getattr(config, "num_predict", 512),
        request_timeout=getattr(config, "request_timeout", 300.0),
    )

    try:
        answer = llm_invoke(
            llm,
            prompt.format(context=context, question=question),
        )

        if isinstance(answer, str):
            clean_answer = clean_generated_answer(answer)
            if clean_answer:
                return clean_answer

        return "Je n'ai pas réussi à générer une réponse claire."

    except Exception as exc:
        details = str(exc).strip() or exc.__class__.__name__
        raise RuntimeError(
            f"Erreur génération LLM ({config.provider}): {details}"
        ) from exc
